...
static NSString *MessageHandlerName = @"RnAliBCWebView";
static NSURLCredential* clientAuthenticationCredential;
static NSDictionary* customCertificatesForHost;

// runtime trick to remove WKWebView keyboard default toolbar
// see: http://stackoverflow.com/questions/19033292/ios-7-uiwebview-keyboard-issue/19042279#19042279
@interface _Hs_SwizzleHelperWK : UIView
@property (nonatomic, copy) WKWebView *webView;
@end
@implementation _Hs_SwizzleHelperWK
...
@interface Hs_RNAliBCWebview () <AlibcBridgeWebViewDelegate, WKUIDelegate, WKNavigationDelegate, WKScriptMessageHandler, UIScrollViewDelegate, RCTAutoInsetsProtocol>
@property (nonatomic, copy) RCTDirectEventBlock onTradeSuccess;
@property (nonatomic, copy) RCTDirectEventBlock onTradeFailure;
...
@implementation Hs_RNAliBCWebview
{
...
- (void)didMoveToWindow
{
    if (self.window != nil && _webView == nil) {
        ...
        if(self.useSharedProcessPool) {
            wkWebViewConfig.processPool = [[HS_AliBCWKProcessPoolManager sharedManager] sharedProcessPool];
        }
        ...
        if (_messagingEnabled) {
            [wkWebViewConfig.userContentController addScriptMessageHandler:[[RNCWeakScriptMessageDelegate alloc] initWithDelegate:self] name:MessageHandlerName];
...
- (void)setSource:(NSDictionary *)source
{
    if (![_source isEqualToDictionary:source]) {
        _source = [source copy];
        //注意先后顺序 不能改
        [self setAliBCParam:[RCTConvert NSDictionary:source[@"aliBCParam"]]];
...
- (void)setAliBCParam:(NSDictionary *)aliBCParam
{
    if (![_aliBCParam isEqualToDictionary:aliBCParam] && aliBCParam) {
        NSString *jsObjectName = [RCTConvert NSString:aliBCParam[@"jsObjectName"]];
        if(jsObjectName){
            MessageHandlerName = jsObjectName;
        }
        _aliBCParam = [aliBCParam copy];
    }
}
...
- (void)visitSource
{
    ...
    NSString *tbUrl = [RCTConvert NSString:_source[@"tbUrl"]];
    if (tbUrl) {
        [RNAlibcBridge sharedInstance].webViewDelegate = self;
        [[RNAlibcBridge sharedInstance] openByUrl_inWebView:_webView url:tbUrl param:_aliBCParam withResolver:nil rejecter:nil];
        return;
    }
    
    NSString *tbCode = [RCTConvert NSString:_source[@"tbCode"]];
    if (tbCode) {
        [RNAlibcBridge sharedInstance].webViewDelegate = self;
        NSInteger pageType = [RCTConvert NSInteger:_source[@"pageType"]];
        [[RNAlibcBridge sharedInstance] openByBizCode_inWebView:_webView type:pageType ? pageType : 0 param:_aliBCParam withResolver:nil rejecter:nil];
...
-(void)setHideKeyboardAccessoryView:(BOOL)hideKeyboardAccessoryView
{
    ...
    NSString* name = [NSString stringWithFormat:@"%@_Hs_SwizzleHelperWK", subview.class.superclass];
    ...
    if(newClass == nil)
    {
        ...
        Method method = class_getInstanceMethod([_Hs_SwizzleHelperWK class], @selector(inputAccessoryView));
...
- (void)layoutSubviews
{
    ...
    // Ensure webview takes the position and dimensions of Hs_RNAliBCWebview
...
#pragma mark - AlibcBridgeDelegate methods

- (void)tradeSuccess:(id)body{
    
    if (_onTradeSuccess) {
        NSMutableDictionary<NSString *, id> *event = [self baseEvent];
        [event addEntriesFromDictionary: body];
        _onTradeSuccess(event);
    }
}

- (void)tradeFailure:(id)body{
    
    if (_onTradeFailure) {
        NSMutableDictionary<NSString *, id> *event = [self baseEvent];
        [event addEntriesFromDictionary: body];
        _onTradeFailure(event);
    }
}
