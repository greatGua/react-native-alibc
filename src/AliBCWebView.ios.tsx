...
import {
  ...
  ViewManager,
  State,
  RNAliBCWebViewUIManagerIOS,
} from './AliBCWebViewTypes';

import styles from './AliBCWebView.styles';

const UIManager = NotTypedUIManager as RNAliBCWebViewUIManagerIOS;
...
const RNAliBCWebViewManager = (NativeModules.RNAliBCWebViewManager || NativeModules.Hs_AliBCWebViewManager) as ViewManager;

const RNAliBCWebView: typeof NativeWebViewIOS = requireNativeComponent(
  'RNAliBCWebView',
);
...
class WebView extends React.Component<IOSWebViewProps, State> {
  ...
  getCommands = () => UIManager.getViewManagerConfig('RNAliBCWebView').Commands;
  ...
  onTradeSuccess = (event: WebViewNavigationEvent) => {
    const { onTradeSuccess } = this.props;
    if (onTradeSuccess) {
      onTradeSuccess(event);
    }
  };

  onTradeFailure = (event: WebViewNavigationEvent) => {
    const { onTradeFailure } = this.props;
    if (onTradeFailure) {
      onTradeFailure(event);
    }
  };
  ...
  render() {
    ...
    if (this.state.viewState === 'LOADING') {
      ...
      console.error(
        `RNAliBCWebView invalid state encountered: ${this.state.viewState}`,
      );
    }
    ...
    const webView = (
      <NativeWebView
        ...
        onTradeSuccess={this.onTradeSuccess}
        onTradeFailure={this.onTradeFailure}
