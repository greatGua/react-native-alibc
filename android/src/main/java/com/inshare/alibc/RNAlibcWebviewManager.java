...
import com.inshare.alibc.events.TopHttpErrorEvent;
import com.inshare.alibc.events.TopLoadingErrorEvent;
import com.inshare.alibc.events.TopLoadingFinishEvent;
import com.inshare.alibc.events.TopLoadingProgressEvent;
import com.inshare.alibc.events.TopLoadingStartEvent;
import com.inshare.alibc.events.TopMessageEvent;
import com.inshare.alibc.events.TopShouldStartLoadWithRequestEvent;
import com.inshare.alibc.events.TopTradeFailureEvent;
import com.inshare.alibc.events.TopTradeSuccessEvent;
...
import static com.inshare.alibc.RNAlibcModule.getShowParams;
import static com.inshare.alibc.RNAlibcModule.getTaokeParam;
import static com.inshare.alibc.RNAlibcModule.getTrackParams;
 ...
 */
@ReactModule(name = RNAlibcWebviewManager.REACT_CLASS)
public class RNAlibcWebviewManager extends SimpleViewManager<WebView> {
    ...
    protected static final String REACT_CLASS = "RNAliBCWebView";
    protected static final String HTML_ENCODING = "UTF-8";
    protected static final String HTML_MIME_TYPE = "text/html";
    protected static String JAVASCRIPT_INTERFACE = "RnAliBCWebView";
    ...
    private ReadableMap aliBCMap = null;
    private ThemedReactContext currentContext = null;

    public RNAlibcWebviewManager() {
    ...
    public RNAlibcWebviewManager(WebViewConfig webViewConfig) {
    ...
    protected WebView createViewInstance(ThemedReactContext reactContext) {
        ...
        webView.setDownloadListener(new DownloadListener() {
            public void onDownloadStart(String url, String userAgent, String contentDisposition, String mimetype, long contentLength) {
                RNAlibcWebviewModule module = getModule(reactContext);
    ...
    @ReactProp(name = "aliBCParam")
    public void aliBCParam(WebView view, @NotNull ReadableMap param) {
        aliBCMap = param;
        Hs_Map paramDict = new Hs_Map(param);
        JAVASCRIPT_INTERFACE = paramDict.optString("jsObjectName",JAVASCRIPT_INTERFACE);
    }
    ...
    public void setSource(WebView view, @Nullable ReadableMap source) {
        if (source != null) {
            ...
            if(source.hasKey("aliBCParam")) {
                aliBCMap = source.getMap("aliBCParam");
                Hs_Map paramDict = new Hs_Map(aliBCMap);
                JAVASCRIPT_INTERFACE = paramDict.optString("jsObjectName", JAVASCRIPT_INTERFACE);
            }
            if (source.hasKey("tbUrl")) {
                String tbUrl = source.getString("tbUrl");
                openByUrl_inWebView(view, tbUrl);
                return;
            }

            if (source.hasKey("tbCode")) {
                String tbCode = source.getString("tbCode");
                openByBizCode_inWebView(view, tbCode, source.hasKey("pageType") ? source.getInt("pageType") : PAGETYPE_DETAIL);
                return;
            }
        }
    ...
    private void openByUrl_inWebView(WebView webView,String tbUrl) {
        Hs_Map paramDict = new Hs_Map(aliBCMap);
        if(TextUtils.isEmpty(tbUrl)){
            return;
        }
        AlibcShowParams showParams = getShowParams(aliBCMap);
        //设置淘客参数
        AlibcTaokeParams taokeParams = getTaokeParam(aliBCMap);
        //第三方用于跟踪的isvcode
        Map<String, String> exParams = getTrackParams(aliBCMap);
        String kitName = paramDict.optString("identity","");
        AlibcTrade.openByUrl(currentContext.getCurrentActivity(), kitName, tbUrl, webView, new RNCWebViewClient(), mWebChromeClient, showParams, taokeParams, exParams, new AlibcTradeCallback() {
            @Override
            public void onTradeSuccess(AlibcTradeResult aliTradeResult) {
                switch (aliTradeResult.resultType){
                    case TYPECART:
                        dispatchEvent(
                                webView,
                                new TopTradeSuccessEvent(
                                        webView.getId(),
                                        successBack(webView, true, 0,"加购成功")));
                        break;
                    case TYPEPAY:
                    ...
                    default:
                        dispatchEvent(
                                ...
                                new TopTradeSuccessEvent(
                                        webView.getId(),
                                        successBack(webView, true, 0,"未知操作，操作成功")));
                        break;
                }
            }
            @Override
            public void onFailure(int i, String s) {
                dispatchEvent(
                        webView,
                        new TopTradeFailureEvent(
                                webView.getId(),
                                successBack(webView, false,i,s)));
            }
        });
    }

    private void openByBizCode_inWebView(WebView webView,String code, int type) {
        Hs_Map paramDict = new Hs_Map(aliBCMap);
        ...
        AlibcTrade.openByBizCode(currentContext.getCurrentActivity(), page, webView, new RNCWebViewClient(), mWebChromeClient, code, showParams, taokeParams, exParams, new AlibcTradeCallback() {
            @Override
            public void onTradeSuccess(AlibcTradeResult aliTradeResult) {
                switch (aliTradeResult.resultType){
                    case TYPECART:
    ...
    private WritableMap ordersBack(WebView webView, List<String> ordersId) {
        WritableMap event = Arguments.createMap();
        event.putDouble("target", webView.getId());
        event.putBoolean("status", true);
        event.putArray("ordersId", (WritableArray) ordersId);

        return event;
    }

    protected WritableMap successBack(WebView webView,boolean status, int i, String msg) {
        WritableMap event = Arguments.createMap();
        event.putDouble("target", webView.getId());
        event.putBoolean("status", status);
        event.putInt("code", i);
    ...
    public Map getExportedCustomDirectEventTypeConstants() {
        ...
        export.put(TopTradeFailureEvent.EVENT_NAME, MapBuilder.of("registrationName", "onTradeSuccess"));
        export.put(TopTradeFailureEvent.EVENT_NAME, MapBuilder.of("registrationName", "onTradeFailure"));
        return export;
    }
    ...
    public static RNAlibcWebviewModule getModule(ReactContext reactContext) {
        return reactContext.getNativeModule(RNAlibcWebviewModule.class);
    }
